 ...
 */
export function escape_markdown(input: string): string {
	return input.replace(FORMATTING_REGEX, "\\$&");
}

export function make_inline_codeblock(input: string): string {
	return "``" + input.replaceAll("`", ZWSP + "`" + ZWSP) + "``";
}

export function make_multiline_codeblock(input: string): string {
