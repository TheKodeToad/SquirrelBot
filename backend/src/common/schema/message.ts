...
function embed<T extends z.ZodType>(stringType: (markdown: boolean) => T) {
	return z
		.strictObject({
			...
			timestamp: z
				.instanceof(TomlDate)
				.transform((date) => date.toISOString()), // TODO is this filter consistent with Discord's
			color: Color,
			footer: z.strictObject({
				text: stringType(false),
				icon: stringType(false).optional(),
			}),
...
export type MessageTemplate<T extends m.Shape = any> = z.infer<
	ReturnType<typeof messageTemplate<T>>
>;

export function messageTemplate<T extends m.Shape>(shape: T) {
	const result = message((markdown) => zTemplate(shape, markdown)).transform(
		(template) => {
			return {
...
function renderMessageTemplate<T extends Shape>(
	template: Message<ZTemplate<T>>,
	view: InferView<T>,
) {
	return {
		...template,
		...
		embeds: template.embeds?.map((embed) =>
			applyEmbedTemplate(embed, view),
		),
	};
}

function applyEmbedTemplate<T extends Shape>(
	template: Embed<ZTemplate<T>>,
	view: InferView<T>,
) {
	const author =
