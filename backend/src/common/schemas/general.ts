...
export const Snowflake = z
	.string()
	.refine(isSnowflake, { error: "Invalid Discord ID (AKA snowflake)" });
...
export function mappedEnum<I extends string, O>(
	map: Readonly<Record<I, O>>,
	params?: string | $ZodEnumParams,
) {
	return z
		.enum(Object.keys(map) as I[], params)
		.transform((input) => map[input]);
}
