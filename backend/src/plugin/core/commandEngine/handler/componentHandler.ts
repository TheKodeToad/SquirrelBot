...
async function handle(
	...
	try {
		const values =
			"values" in interaction.data ? interaction.data.values.raw : undefined;
		...
		try {
			await ctx.respond({
				components: [
					Text(":boom: Something went wrong while processing your action"),
				],
...
export function listenForInteractions(
	messageID: string,
	originalUserID: string,
	...
	async respond(reply: Reply): Promise<void> {
		...
		if (this._responseID !== null) {
			...
			await this._interaction
				.editFollowup(this._responseID, messageOptions)
			...
			if (this._edited) {
				await this._interaction
					.createFollowup(messageOptions)
					.then((response) => (this._responseID = response.message.id));
			} else {
				await this._interaction
