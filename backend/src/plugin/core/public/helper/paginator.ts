	...
	reply.componentHandler = async (ctx, customID, ...remaining) => {
		if (ctx.originalUserID === ctx.user.id) {
			if (customID === "paginator-prev") {
				...
				const before =
					firstItem !== undefined ? paginator.getKey(firstItem) : undefined;

				await ctx.edit(
					await renderPaginator(ctx, paginator, true, before, undefined),
				);
				return;
			} else if (customID === "paginator-next") {
				const lastItem = queryResult[queryResult.length - 1];
				const after =
					lastItem !== undefined ? paginator.getKey(lastItem) : undefined;

				await ctx.edit(
					await renderPaginator(ctx, paginator, false, undefined, after),
				);
				return;
			}
		}
