				...
				fields: [
					{
						name: "Name",
						value:
							"{{#name_changed}}{{old_role.name}} → {{new_role.name}}{{/name_changed}}",
					},
					{
						name: "Color",
						value:
							"{{#color_changed}}{{old_role.color}} → {{new_role.color}}{{/color_changed}}",
					},
					{
						name: "Hoisted",
						value:
							"{{#hoisted_changed}}{{old_role.hoisted}} → {{new_role.hoisted}}{{/hoisted_changed}}",
					},
					{
						name: "Mentionable",
						value:
							"{{#mentionable_changed}}{{old_role.mentionable}} → {{new_role.mentionable}}{{/mentionable_changed}}",
					},
				],
				color: "yellow",
				footer: { text: "Actor ID: {{actor.id}}" },
