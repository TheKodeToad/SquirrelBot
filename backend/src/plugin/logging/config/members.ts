...
export const MemberJoinEvent = eventConfig(
	messageTemplate(m.object({ user: UserView })),
	{
		embeds: [{
			...
			fields: [
				{ name: "Account Created At", value: "{{#user.created_at}}{{.}} ({{user.age}} old){{/user.created_at}}" }
			],
			color: "green",
...
export const MemberLeaveEvent = eventConfig(
	messageTemplate(m.object({ user: UserView })),
	{
		embeds: [{
			...
			author: { name: "{{user.tag}}", icon_url: "{{user.avatar}}" },
			fields: [
				{ name: "Joined At", value: "{{#user.joined_at}}{{.}} (stayed for {{user.membership_duration}}){{/user.joined_at}}" },
			],
			color: "red",
			footer: { text: "User ID: {{user.id}}" },
		}]
